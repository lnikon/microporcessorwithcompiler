...
Tokenizer::Tokenizer(std::string expression) {
	...
	expression.erase(
			std::remove(std::begin(expression), std::end(expression),
					char(' ')), std::end(expression));
	...
	while ((pos = expression.find_first_of(delimeters, pos))
			!= std::string::npos) {
...
auto Tokenizer::isStringNumber(const std::string& str) {
	return !str.empty()
...
auto Tokenizer::searchNumber(std::string::iterator begin,
